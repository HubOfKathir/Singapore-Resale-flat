...
#setup page config
st.set_page_config(page_title='HDB flat price predictor',page_icon='house',initial_sidebar_state='expanded',
                   layout='wide',menu_items = {"about":'This Streamlit application was developed by Kathir S'})
...
#setup the sidebar with optionmenu
with st.sidebar:
    selected = option_menu(menu_title="Main Menu",options=['Home','Get Prediction','About'],
                           icons=['house','lightbulb','info-circle'],
                           default_index=0,
                           orientation='vertical',
                           styles={'container':{'padding':'5px','border':'2px ridge','background-color':'#002g36'},
...
if selected == 'Get Prediction':
    ...
    with st.form('prediction'):
        ...
        with col2:
            ...
            with c2:
                storey_end = st.number_input(label='Storey end',min_value=1,max_value=50)
    ...
    if button:
        with st.spinner('Predicting...'):
            ...
            if not all([user_month,user_town,user_flat_type,user_flat_model,floor_area_sqm,price_per_sqm,year,block,
                ...
                with open('Decisiontrees.pkl','rb') as files:
